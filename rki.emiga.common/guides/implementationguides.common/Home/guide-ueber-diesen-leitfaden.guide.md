# {{page-title}}
Dieser Implementierungsleitfaden beschreibt die FHIR-Spezifikation des EMIGA-Common-Pakets <a href="https://simplifier.net/rki.emiga.common">rki.emiga.common</a>. Er umfasst die grundlegenden FHIR-Profile, Terminologien und weiteren Artefakte, die paketübergreifend in EMIGA verwendet werden und die Basis für den standardisierten Austausch und die Verarbeitung von Daten über die FHIR-Schnittstelle gemäß dem definierten Datenmodell bilden. Dazu gehören unter anderem die Modellierung der betroffenen Person, Identifier, Bearbeitungsstatus, Annotationen, Anhänge sowie weitere gemeinsame Datenelemente. Darüber hinaus erläutert dieser Leitfaden die im Paket rki.emiga.common enthaltenen Inhalte. Zusammen mit den weiteren EMIGA-Implementierungsleitfäden bildet dieser Leitfaden die Grundlage für einen interoperablen und standardisierten Datenaustausch im Rahmen von EMIGA.

### Zielgruppe
Dieser Leitfaden richtet sich an Anwendende sowie Softwareherstellende von Informationssystemen, die die Daten gemäß der EMIGA-Spezifikation an EMIGA übermitteln, aus EMIGA empfangen oder innerhalb von EMIGA verarbeiten.

### Aufbau dieses Implementierungsleitfadens
Dieser Leitfaden enthält unter anderem:

- die Definition grundlegender FHIR-Profile (*StructureDefinitions*)
- Definitionen der verwendeten *FHIR-Extensions*
- Definitionen von *OperationDefinitions*
- semantische Vorgaben in Form von *ValueSets*, *CodeSystems* und *NamingSystems* (siehe Abschnitt Terminologien)
- *CapabilityStatements* zur Beschreibung der unterstützten FHIR-Schnittstellen
- Informationen zu den *Abhängigkeiten* von den zugrunde liegenden FHIR-Basisprofilen und externen Spezifikationen (z.B. DEMIS)

Hinweise und Beispiele zur Anwendung dieser Ressourcen finden sich in den Erklärungen zu den einzelnen Ressourcen.